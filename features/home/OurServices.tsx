...
const servicesOriginalWidth = 60;
const addWidth = 30;
      ...
      <motion.div
        ...
        <div className="flex flex-col justify-center items-center pb-8 pt-2 ">
          <div className="pb-32 flex w-full">
            ...
            >
              <Header name="Our services" type="h2" className="text-center text-3xl" />
            </motion.div>
          </div>
            ...
            <AnimatePresence mode="wait" >
              {serviceDetails.map((service) => (
                ...
                >
                  ...
                  <motion.div
                    className="w-full h-[175px] relative overflow-hidden"
                  >
                    {service.images.map((image, index) => (
                      <motion.div
                        key={index}
                        className="absolute right-0 top-0 h-[175px] origin-right"
                        animate={{
                          width:
                            hoveredImage === service.id
                              ? getWidth(index, 1) * 1.6 // expanded on hover
                              : getWidth(index, 1),      // normal size
                        }}
                        // Only animate on hover-in; make hover-out instant (no zoom-out animation)
                        transition={hoveredImage === service.id ? { duration: 0.8, ease: "easeInOut" } : { duration: 0.8 }}
                        style={{
                          zIndex: index,
                        }}
                      >
                        <Image
                          src={hoveredImage === service.id ? image.hoverSrc : image.src}
                          alt={`img${index + 1}`}
                          width={getWidth(index, 1)}
                          height={175}
                          className="object-cover h-[175px] w-full rounded-tr-[15px] rounded-br-[15px]"
                        />
                      </motion.div>
